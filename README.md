...
Under all of it is the **bed** — a track from `audio/`, trimmed to the reel's length
and faded out at the end. The signature track is the default on every reel;
`music.file` in a site's config swaps it for another checked-in track and
`music.offset` slides it. Nothing is timed to the music: a reel is exactly as long
with a bed as it is without one, whichever track it is.
